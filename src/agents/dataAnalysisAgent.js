...
function sampleEvents(events, count = 20) {
...
export async function detectSchemaWithLLM(events) {
 ...
 */
export function buildFieldMapping(schema) {
 ...
 */
export function getFieldValue(event, fieldName) {
 ...
 */
export function createFieldMappingConfig(schema) {
 ...
 */
export function processEventsMinimal(rawEvents, fieldMapping) {
  ...
  const events = rawEvents.map(ev => {
    ...
    let startValue = getFieldValue(raw, fieldMapping.start) 
      ?? getFieldValue(ev, fieldMapping.start)
      ?? ev.enter?.Timestamp ?? ev.Timestamp;
    ...
    if (endValue === null || endValue === undefined) {
      const durValue = getFieldValue(raw, fieldMapping.duration) 
        ?? getFieldValue(ev, fieldMapping.duration) ?? 0;
    ...
    const tid = String(
      getFieldValue(raw, fieldMapping.tid) ?? 
      getFieldValue(ev, fieldMapping.tid) ?? 
      pid
    );
    ...
    const levelRaw = getFieldValue(raw, fieldMapping.level) ?? getFieldValue(ev, fieldMapping.level) ?? 0;
    const level = Number.isFinite(Number(levelRaw)) ? Number(levelRaw) : 0;
    
    const name = getFieldValue(raw, fieldMapping.name) ?? getFieldValue(ev, fieldMapping.name) ?? '';
    const cat = getFieldValue(raw, fieldMapping.cat) ?? getFieldValue(ev, fieldMapping.cat) ?? '';
    const args = getFieldValue(raw, fieldMapping.args) ?? getFieldValue(ev, fieldMapping.args) ?? {};
 ...
 */
export function generateInitialConfig(schema, fieldMapping) {
...
export async function analyzeAndInitialize(rawEvents) {
  ...
  try {
    ...
    return {
      ...
      fieldMapping: {
        ...
        pid: 'pid', tid: 'tid', start: 'ts', duration: 'dur',
