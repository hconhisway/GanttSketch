...
export const YAXIS_CONFIG_SCHEMA = {
  hierarchy1OrderRule: {
    type: 'object',
    description: 'Rule for ordering hierarchy-1 rows on the Y-axis.',
    examples: [
      // Fork tree (parent-child hierarchy)
      { type: 'transform', name: 'forkTree', params: { includeUnspecified: true } },
      // Sort by hierarchy1 key ascending
      { type: 'transform', name: 'pidAsc' },
      // Sort by hierarchy1 key descending
  ...
  hierarchy2LaneRule: {
    type: 'object',
    description: 'Rule for arranging hierarchy-2 lanes within a hierarchy-1 row. Default: autoPack. Use byField with params.field to link any event attribute.',
    examples: [
      { type: 'transform', name: 'autoPack' },
      { type: 'transform', name: 'byField', params: { field: 'level' } },
...
export const WIDGET_API_REFERENCE = {
  ...
  setProcessSortMode: {
    description: 'Set hierarchy1 sorting mode',
