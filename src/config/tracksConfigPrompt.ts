...
export const PREDEFINED_FILTER_FUNCTIONS: Record<string, ((track: string) => boolean) | null> = {
 ...
 */
export function parseTrackConfigFromResponse(responseText: string) {
  try {
 ...
 */
export function convertLLMConfigToTracksConfig(llmConfig: any, data: any[]) {
      ...
      case 'pattern':
        {
          ...
          result.filter = (track: string) => regex.test(track.toString());
        }
        break;
      ...
      case 'function':
        {
          ...
          if (funcName === 'top_n_utilization') {
            ...
            const trackStats: Record<string, { sum: number; count: number }> = {};
 ...
 */
export function getEnhancedSystemPrompt(chartContext: any) {
